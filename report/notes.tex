...
\be
  Z = \text{Tr}[\tk^N]
\ee
Instead of multiplying the transfer matrix N - 1 times and finding the trace of the result, we can diagonalize \tk by performing an eigendecomposition.
\be
...
\be
  Z = \lambda_1^N + \lambda_2^N = \lambda_1^N \Big(1 + \frac{\lambda_2^N}{\lambda_1^N} \Big) \xrightarrow[N \rightarrow \infty]{} \lambda_1^N .
\ee
